/**
 * Airplane Model
 * Creates and manages the airplane model and its animations
 */
...
function createPlane() {
  ...
  airplane.mesh.position.x = -40; // Center the plane more by moving it to the left
  scene.add(airplane.mesh);
}

// Update airplane position and rotation
function updatePlane() {
  // Update mousePos from keyboard if keys are pressed
  updateMousePosFromKeyboard();

  game.planeSpeed = normalize(mousePos.x, -.5, .5, game.planeMinSpeed, game.planeMaxSpeed);
  const targetY = normalize(mousePos.y, -.75, .75, game.planeDefaultHeight - game.planeAmpHeight, game.planeDefaultHeight + game.planeAmpHeight);
  ...
  const adjustedTargetY = targetY + game.planeCollisionDisplacementY;

  airplane.mesh.position.y += (adjustedTargetY - airplane.mesh.position.y) * deltaTime * game.planeMoveSensivity;
  airplane.mesh.position.x += (targetX - airplane.mesh.position.x) * deltaTime * game.planeMoveSensivity;

  airplane.mesh.rotation.z = (adjustedTargetY - airplane.mesh.position.y) * deltaTime * game.planeRotXSensivity;
  airplane.mesh.rotation.x = (airplane.mesh.position.y - adjustedTargetY) * deltaTime * game.planeRotZSensivity;
